...
class m170804_163745_create_china extends Migration
{
    /**
     * @var string 定义表名
     */
    ...
    public function safeUp()
    {
        ...
        if ($this->db->driverName === 'mysql') {
            $tableOptions = 'CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE=InnoDB comment "地址信息表"';
        }

        // 创建表
        $this->createTable($this->table, [
            'id' => $this->integer(11)->notNull()->defaultValue(0)->comment('地址ID'),
            'name' => $this->string(32)->notNull()->comment('地址名称'),
            'pid' => $this->integer(11)->defaultValue(0)->comment('父类ID'),
            'PRIMARY KEY (`id`)',
            'KEY `FK_CHINA_REFERENCE_CHINA` (`pid`)',
            'CONSTRAINT `FK_CHINA_REFERENCE_CHINA` FOREIGN KEY (`pid`) REFERENCES ' . $this->table . ' (`id`)'
        ], $tableOptions);

        $array = array_chunk($this->array, 500);
        foreach ($array as $value) {
            $this->batchInsert($this->table, ['id', 'name', 'pid'], $value);
        }
    }
